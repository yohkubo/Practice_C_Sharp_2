    ...
    public partial class Default : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            while (heroHealth > 0 && monsterHealth > 0)
            {
                ...
                result += String.Format("<br />Monster causes {0} damage, leabing hero with {1} health.",
                    monsterDamage, heroHealth);      
            }
            */

            // At least monster can attack hero one time even if monster was killed by hero's first bonus attack
            do
            {
                int heroDamage = random.Next(1, 10);
                int monsterDamage = random.Next(1, 20);
